...
Function ExportScatterData(positions, xSize, ySize, pixelSize, FilteringType, imageFileLocation)
	...
	Make /D/O Scatter_X, Scatter_Y, Scatter_Color
	Make /I/O Filter_X, Filter_Y, Filter_Color
...
Function ExtractTracksToScatter(positions, Scatter_X, Scatter_Y)
	...
	for (TrackItor = 0; TrackItor < nTracks; TrackItor++)
		...
		//Print "There are: " + num2str(DimSize(CurrentPosition, 0)) + " points in the current track and " + num2str(nTracks) + " Tracks Overall"
		...
		// itorate over the individual points in the tracks
		for(PointItor = 0; PointItor < DimSize(CurrentPosition,0); PointItor++)
		Print "Time Index: " + num2str(CurrentPosition[PointItor][zCol]) + " X " + num2str(CurrentPosition[PointItor][xCol]) + " Y " + num2str(CurrentPosition[PointItor][yCol])
...
Function RetrieveIntensityPlot(Filter_X, Filter_Y, Filter_Frame, PointIndex)
	...
	if((Filter_X[PointIndex] > 0)&&(Filter_Y[PointIndex] > 0)&&(Filter_X[PointIndex] < (image_width - 1))&&(Filter_X[PointIndex] < (image_height - 1)))
		...
		for(itor_X = -1; itor_X < 2; itor_X++)
			for(itor_Y = -1; itor_Y < 2; itor_Y++)
				ImageTransform/beam={(Filter_X[PointIndex] + itor_X), (Filter_Y[PointIndex] + itor_Y)} getbeam ChannelOne
				wave W_Beam
...
Function StoreIndividualIntensityWave(point_X, point_Y, average, RGB_Value, PointIndex)
	...
	//------------------------------
	String IntensityData_Name = "X" + num2str(point_X) + "Y" + num2str(point_Y)
	String BaselineIntensityData_Name = "X" + num2str(point_X) + "Y" + num2str(point_Y) + "B"	
	String Frame_Data_Name = "X" + num2str(point_X) + "Y" + num2str(point_Y) + "C"
	//---------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------------------------------------------------
// section associated with the taking data output saved as integer and exporting the decimal points calculated by localizer 
//-------------------------------------------------------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------------------------------------------------



Function DecimalizePanel(positions)
	wave positions
	...
	if(DataFolderExists("root:Decimalize"))
		SetDataFolder root:Decimalize
		KillWaves/A/Z
	else
		NewDataFolder root:Decimalize
		SetDataFolder root:Decimalize
	endif

	//---------------------------------------------------------------------------------------------
	// load the x and y coordinates into integer storage locations 
	//---------------------------------------------------------------------------------------------
	Make /I/O/N=0 Integer_X, Integer_Y
	LoadCurveFitDecimalized()

	//---------------------------------------------------------------------------------------------
	// get the x and y coordinates from the main localizer utility for comparison to the text tile
	//---------------------------------------------------------------------------------------------

	variable nPositions = DimSize(positions, 0)
	// create list to store the X, Y
	Make /D/O/N=(nPositions) Scatter_X, Scatter_Y, Scatter_Color
	// get the indices of the columns containing the x and y coordinates
	variable xCol, yCol, zCol
	getColumnsForEmitterPositions(positions, xCol, yCol, zCol)
	if ((xCol == -1) || (yCol == -1))
	...
	//---------------------------------------------------------------------------------------------------------------	
	// create the integer equivalent of the localizer waves in the same order for searching one list against another
	//---------------------------------------------------------------------------------------------------------------
	Make /I/O/N=(nPositions) Localizer_X, Localizer_Y

	// direct copy, losing the decimal information 
	variable itor	
	for(itor = 0; itor < nPositions; itor++)
	// specified the *floor* not the rounded data
		Localizer_X[itor] = floor(Scatter_X[itor])
		Localizer_Y[itor] = floor(Scatter_Y[itor])
	endfor
	//-----------------------------------------------------------------------------------------------
	//create floating point wave to store the matching integer waves points and fill using a search
	//-----------------------------------------------------------------------------------------------
	Make /D/O/N=(numpnts(Integer_X)) Matching_X, Matching_Y
	PointSearch()

	// save the decimalized points in direct comparison to the integer points
	
	String/G DecimalWaveNames = "Integer_X;Integer_Y;Matching_X;Matching_Y"
	Save/J/B DecimalWaveNames

End

Function PointSearch()
	// data read in from text file
	wave Int_X = root:Decimalize:Integer_X
	wave Int_Y = root:Decimalize:Integer_Y
	// data recalculated from localizer
	wave Local_X = root:Decimalize:Localizer_X
	wave Local_Y = root:Decimalize:Localizer_Y
	// raw decimal data
	wave Scatter_Local_X = root:Decimalize:Scatter_X
	wave Scatter_Local_Y = root:Decimalize:Scatter_Y
	// list of points found
	wave Found_X = root:Decimalize:Matching_X
	wave Found_Y = root:Decimalize:Matching_Y

	// will only include the first point found, the original filter used looks only for the first instance
	variable itor_point, itor_list
	variable point_found = 0
	for(itor_point = 0; itor_point < numpnts(Int_X); itor_point++)
	Print "Seaching point " + num2str(itor_point) + " of " + num2str(numpnts(Int_X))
		for(itor_list = 0; itor_list < numpnts(Local_X); itor_list++)
			if(point_found > 0.5)
				continue
			endif	
			if((Int_X[itor_point] == Local_X[itor_list])&&(Int_Y[itor_point] == Local_Y[itor_list]))
				
				Found_X[itor_point] = Scatter_Local_X[itor_list]
				Found_Y[itor_point] = Scatter_Local_Y[itor_list]
				Print "Point Located"
				// set point found to 1 so that the rest of the loop is skipped
				point_found = 1
			Endif
		Endfor
		// reset point found so that the next point can be searched
		point_found = 0
	Endfor
End
...
Function LoadCurveFitDecimalized()
	...
	if(numtype(strlen(TextFilePath)) != 2)
		...
		LoadWave /A/H/J/K=1/O/W TextFilePath
			...
			Wv1 = WaveExists(root:Decimalize:XW)
			Wv2 = WaveExists(root:Decimalize:YW)
			
			if(Wv1 && Wv2)
				Print "Made it here"
				Redimension /N=(numpnts(root:Decimalize:XW)) Int_X
				Redimension /N=(numpnts(root:Decimalize:YW)) Int_Y
				// duplicate the proper waves  	
				Duplicate /O root:Decimalize:XW Int_X
				Duplicate /O root:Decimalize:YW Int_Y
				// now concatenate the x wave and y wave on to the empty integer waves
				//Concatenate /NP/O {Int_X,root:Decimalize:XW}, Int_X
				//Concatenate /NP/O {Int_Y,root:Decimalize:YW}, Int_Y
			else
				DoAlert /T="File Load Error" 0, "This file does not appear to contain X Y coordinates"
			Endif
...
//functions associated with subtracting images from one-another in post processing after the points have been localized

Function SubtractBackgroundPanel()

	svar ChannelOneReference

	// open the image to subtract from the current data set 
	// dialog first	

	Variable refNum
	String message = "Select a tiff file"
	String/G ChannelTwoPath
	String fileFilters = "Tiff Files (*.tif,*.tiff):.tif,.tiff;"


	Open /D /R /F=fileFilters /M=message refNum
	
	ChannelTwoPath = S_FileName



	// open as an image
	if(numtype(strlen(ChannelTwoPath)) != 2)	

		ImageLoad/T=tiff/S=0/C=-1/Q ChannelTwoPath
		String/G ChannelTwoReference = StringFromList(0,s_waveNames)
	
		//check to make sure the two images are of equal dimension and time steps
		...
		if( (Ch1_width == Ch2_width)&&(Ch1_height == Ch2_height)&&(Ch1_frames == Ch2_frames))
			...
			// set up a place to store all of the data and/or clean it up if it already exists (maybe not)
			if(DataFolderExists("root:SubtractionInterface"))
				SetDataFolder root:SubtractionInterface
				KillWaves/A/Z
				SetDataFolder root:
			else
				NewDataFolder root:SubtractionInterface
				SetDataFolder root:
			endif


			OpenSubtractionPanel()
		else
			Print "The Second channel's dimensions do not match the first channel"
		endif
...
Function OpenSubtractionPanel()
	...
	String/G ChannelOneCropReference = "Ch1Crop"
	String/G ChannelTwoCropReference = "Ch2Crop"
	
	SetDataFolder root:SubtractionInterface:
		// these variables have already been compared for the two images and are equal
		// needed for the slider and image control 
	
		// keeping track of the size of the image in the X, Y, and frame dimensions
		Variable/G Channel_width = dimsize(ChannelOne, 0)
		Variable/G Channel_height = dimsize(ChannelOne, 1)
		Variable/G Channel_frames = dimsize(ChannelOne, 2)

		// keeping track of which frame is displayed in the images
		Variable/G Current_frame = 0

		// keeping track of the current point we are looking at and the images are centered on, 
		//starting from the first accepted in the list
		Variable/G CurrentPointIndex = GetFirstAcceptedIndex()
		assert(CurrentPointIndex > -0.5)

		// keeping track of the average intensity traces associated with channel one and channel two a point X,Y
		Make/O/N=(dimsize(ChannelOne, 2)) ChannelOne_average
		Make/O/N=(dimsize(ChannelTwo, 2)) ChannelTwo_average
		// the wave used as output for the math, set to same size as above and zero it for now, flag 
		// lets other functions know if sometheing is really ready to be plotted
		Make/O/N=(dimsize(ChannelTwo, 2)) ChannelResidual
		ChannelResidual = 0
		Variable/G ChannelResidual_exists = 0
		//keeping track of the multiplier value and offset for subtraction purposes
		Variable/G Channel_multiplier = 1
		Variable/G Channel_offset = 100
		
		Variable/G PointIntervalLength = 1000

		// keeping track of the left axis max and min for the cursor plotting function
		Variable/G MaxLeftAxis
		Variable/G MinLeftAxis

		// global variable responsible for setting the zoom subtraction and addition on the pixel we a centered on
		Variable/G PixelDelta = 10

	SetDataFolder root:
	//-------------------------------------------------------
	// create the overall panel that we are working in
	//-------------------------------------------------------
	NewPanel /W=(278,225,1082,620)/N=ChannelSubtraction
	SetDrawLayer UserBack
	//-------------------------------------------------------

	//-------------------------------------------------------------
	// create a button and info windows to navigate through different spots
	//-------------------------------------------------------------
	GroupBox PointNavigator,pos={14.00,5.00},size={427.00,56.00},title="Point Navigator"
	ValDisplay XValDisplay,pos={243.00,30.00},size={70.00,18.00},title="X:"
	ValDisplay XValDisplay,limits={0,0,0},barmisc={0,1000},value= #"root:WaveNameArray_X[root:SubtractionInterface:CurrentPointIndex]"
	ValDisplay YValDisplay,pos={322.00,30.00},size={70.00,18.00},title="Y:"
	ValDisplay YValDisplay,limits={0,0,0},barmisc={0,1000},value= #"root:WaveNameArray_Y[root:SubtractionInterface:CurrentPointIndex]"
	Button PreviousPoint,pos={62.00,30.00},size={70.00,20.00},title="Previous",proc=SubtractionPreviousPoint
	Button NextPoint,pos={141.00,30.00},size={70.00,20.00},title="Next",proc=SubtractionNextPoint
	//-------------------------------------------------------------



	//-------------------------------------------------------------
	// create two spot zoom ins to compare channel areas visually
	//-------------------------------------------------------------
	TitleBox Chan1,pos={86.00,65.00},size={61.00,24.00},title="Channel 1",frame=4
	TitleBox Chan2,pos={308.00,65.00},size={61.00,24.00},title="Channel 2",frame=4

	// using the whole image appears to be too slow, try duplicating and removing extraneous portions
	Duplicate /O/R=[X_Points[CurrentPointIndex] - PixelDelta,X_Points[CurrentPointIndex] + PixelDelta][Y_Points[CurrentPointIndex] - PixelDelta,Y_Points[CurrentPointIndex] + PixelDelta] ChannelOne,$ChannelOneCropReference
	Duplicate /O/R=[X_Points[CurrentPointIndex] - PixelDelta,X_Points[CurrentPointIndex] + PixelDelta][Y_Points[CurrentPointIndex] - PixelDelta,Y_Points[CurrentPointIndex] + PixelDelta] ChannelTwo,$ChannelTwoCropReference

	Display/W=(12,93,223,289)/HOST=#
	AppendImage $ChannelOneCropReference
	ModifyImage $ChannelOneCropReference ctab= {*,*,Grays,0}
	ModifyGraph mirror=0
	RenameWindow #,CH1
	SetActiveSubwindow ##

	Display/W=(232,92,442,288)/HOST=#
	AppendImage $ChannelTwoCropReference
	ModifyImage $ChannelTwoCropReference ctab= {*,*,Grays,0}
	ModifyGraph mirror=0
	RenameWindow #,CH2
	SetActiveSubwindow ##
	//-------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	// create a slider and info/set windows to navigate through frames
	//-----------------------------------------------------------------------
	GroupBox FrameNavigator,pos={14.00,296.00},size={427.00,56.00},title="Frame Navigator"
	Slider TimeSlider,pos={25.00,324.00},size={279.00,10.00},limits={0,(Channel_frames) - 1,1},value= 0,side= 0,vert= 0,proc=ChannelComparisonSlider
	SetVariable FrameSet,pos={315.00,320.00},size={66.00,18.00},limits={0,(Channel_frames) - 1,0},value=root:SubtractionInterface:Current_frame,title=" ",proc=ChannelComparisonSetVar
	ValDisplay MaxFrame,pos={394.00,320.00},size={39.00,18.00},limits={0,0,0},barmisc={0,1000},value=#"root:SubtractionInterface:Channel_frames - 1"
	//-------------------------------------------------------------

	//-------------------------------------------------------------
	// setup of area to display intensity traces and residual at X,Y, add in a line associate with the slider frame location
	//-------------------------------------------------------------
	Get3by3BeamAverage()
	TitleBox IntensityTraces,pos={555.00,18.00},size={156.00,24.00},title="Intensity Traces and Residue",frame=4

	String fldrSav0= GetDataFolder(1)
	SetDataFolder root:SubtractionInterface:
	Display/W=(467,48,788,293)/HOST=#/N=SubtractDisplay  ChannelOne_Average, ChannelTwo_Average 
	SetDataFolder fldrSav0
	RenameWindow #,IntensityTraces
	
	// zoom out by 10 percent on the vertical data to avoid hitting the top and bottom for ease of viewing
	// get the maximum value for the two waves displayed
	variable maxVal = max((wavemax(ChannelOne_average)), (wavemax(ChannelTwo_average)))
	variable minVal = min((wavemin(ChannelOne_average)), (wavemin(ChannelTwo_average)))
	variable range = maxVal - minVal
	variable delta = range * 0.1 / 2.0
	MaxLeftAxis= maxVal + delta
	MinLeftAxis = minVal - delta
	SetAxis /W=ChannelSubtraction#IntensityTraces left,MinLeftAxis,MaxLeftAxis
	// set the line thickness
	ModifyGraph /W=ChannelSubtraction#IntensityTraces lSize=2
	//Set the colors of the different traces
	ModifyGraph /W=ChannelSubtraction#IntensityTraces rgb(ChannelOne_Average)=(65535,0,0)
	ModifyGraph /W=ChannelSubtraction#IntensityTraces rgb(ChannelTwo_Average)=(0,0,0)
	...
	ModifyGraph /W=ChannelSubtraction#IntensityTraces mirror=1,axThick=3,tick=2,btLen=4,btThick=3,minor=0,standoff=0	
	Label bottom "Frame"
	Label left "Intensity"
	
	// now append the residual wave on the right-hand axis, scaled from -1 to 1 (wave should be all zero), scaling to be adjusted as it is updated	
	AppendtoGraph /W=ChannelSubtraction#IntensityTraces /R ChannelResidual
	// color the residual blue for now and as a dashed line
	ModifyGraph /W=ChannelSubtraction#IntensityTraces rgb(ChannelResidual)=(0,0,65535),lstyle(ChannelResidual)=3, lSize(ChannelResidual)=2
	//modify the residual axis to range properly
	SetAxis /W=ChannelSubtraction#IntensityTraces right,-1,1
	// apply the same sets of plot parameters (mostly) as what was used for the two channel portion
	ModifyGraph /W=ChannelSubtraction#IntensityTraces axThick(right)=3,tick(right)=2,btLen(right)=4,btThick(right)=3,minor(right)=0,standoff(right)=0 
	Label right "Residual"



	// for the whole graph
	//set font of axis at bold 12 pt 
	ModifyGraph /W=ChannelSubtraction#IntensityTraces fstyle=1,fsize=12,font="arial",lblMargin(left)=15,lblMargin(right)=15, lblMargin(bottom)=5


	// frame cursor

	
	// start drawing the frame cursor
	SetDrawEnv gstart,gname= FrameCursor
	// color is sort of yellow, line thickness 1, drawing within the graph frame
	SetDrawEnv linefgc=(50000,50000,0),linethick=2,xcoord=bottom,ycoord=left
	// draw the line starting from 0 at first
	DrawLine 0, MinLeftAxis, 0, MaxLeftAxis
	//stop drawing the frame cursor
	SetDrawEnv gstop
	...
	//------------------------------------------------------------------------------------------------------
	//Set up area to input changes to the multiplication and offset(s) of the channels for channel subtraction
	//------------------------------------------------------------------------------------------------------

	SetVariable ChanMult,pos={469.00,308.00},size={193.00,19.00},bodyWidth=66,title="Channel Multiplier (AU)",limits={-inf,inf,0},value=root:SubtractionInterface:Channel_multiplier
	SetVariable TimeOffset,pos={488.00,334.00},size={175.00,19.00},bodyWidth=66,title="Time Offset (msec): ",limits={-inf,inf,0},value=root:SubtractionInterface:Channel_offset
	SetVariable PointInterval,pos={488.00,360.00},size={175.00,19.00},bodyWidth=66,title="Point Interval (msec): ",limits={-inf,inf,0},value=root:SubtractionInterface:PointIntervalLength


	
	Button CurveFit,pos={677.00,308.00},size={110.00,20.00},title="Auto Subtract",proc=AutoSubtract
	Button Reset,pos={677.00,334.00},size={110.00,20.00},title="Reset",proc=ResetSubtract
...
// respond to changes in the frame slider
Function ChannelComparisonSlider(sa) : SliderControl
...
Function ChannelComparisonSetVar(sva) : SetVariableControl
	STRUCT WMSetVariableAction &sva

	variable SetVariableVal

	switch( sva.eventCode )
		case 1: // mouse up
		case 2: // Enter key
		case 3: // Live update
			SetVariableVal = sva.dval
			ChangeImages(SetVariableVal)
...
// modify images to match slider and change the current frame
Function ChangeImages(FrameNumber)
	variable FrameNumber

	nvar Number_Frames = root:SubtractionInterface:Channel_frames
	nvar Frame = root:SubtractionInterface:Current_frame
	svar ChannelTwoCropReference
	svar ChannelOneCropReference

	variable PlaneChoice = FrameNumber

	// check bounds before changing
	if(PlaneChoice > Number_Frames)
		PlaneChoice = Number_Frames - 1
	endif
	...
	// set the plane on the images and also the current frame after boundary check
	ModifyImage/W=ChannelSubtraction#CH1 $ChannelOneCropReference plane=PlaneChoice
	ModifyImage/W=ChannelSubtraction#CH2 $ChannelTwoCropReference plane=PlaneChoice
	//update the slider as well
	Slider TimeSlider win=ChannelSubtraction, value=PlaneChoice

	// update the cursor in the intensity vs frame graph
	 ChangeCursor(PlaneChoice)

	// update the frame for other functions  
	Frame = PlaneChoice


End

// called by change images to modify the graph cursor
Function ChangeCursor(Frame)
	variable Frame
	...
	// delete the previous line and reinsert	
	DrawAction /W=ChannelSubtraction#IntensityTraces getgroup=FrameCursor, delete, begininsert
	SetDrawEnv /W=ChannelSubtraction#IntensityTraces  gstart,gname= FrameCursor
	// color is sort of yellow, line thickness 1, drawing within the graph frame
	SetDrawEnv /W=ChannelSubtraction#IntensityTraces linefgc=(50000,50000,0),linethick=2,xcoord=bottom,ycoord=left
	// draw the line at Frame with Y defined by MaxAxis and MinAxis
	DrawLine /W=ChannelSubtraction#IntensityTraces Frame, MinAxis, Frame, MaxAxis
	SetDrawEnv /W=ChannelSubtraction#IntensityTraces gstop
	//exit insert mode
	DrawAction /W=ChannelSubtraction#IntensityTraces endinsert

End



// find the first point in the list that is accepted and return it
// if no points return -1 
Function GetFirstAcceptedIndex()
	if(WaveExists(root:KeepWaveArray))
		wave/I KeepWaveList = root:KeepWaveArray
		variable itor
...
// return to previous point in subtraction list
Function SubtractionPreviousPoint(CtrlName) : ButtonControl
	String CtrlName

	wave/I KeepWaveList = root:KeepWaveArray

	NVAR CurrentPoint = root:SubtractionInterface:CurrentPointIndex
	// needed for cursor update
	NVAR Frame = root:SubtractionInterface:Current_frame

	variable itor

	// check if we are at the beginning of the list, if so go to the end and count down to the first point that matches
	if(CurrentPoint <= 0)		
		
		for(itor = dimsize(KeepWaveList,0) - 1; itor >= 0; itor--)
			if(KeepWaveList[itor] > 0.5)
				CurrentPoint = itor
				SetPointScales()
				Get3by3BeamAverage()
		...
		// something is wrong if we reach here searched the entire list, crash
		assert(0)
	endif
	
	// not at the end of the list, start countdown at current point
	for(itor = CurrentPoint - 1; itor >= 0; itor--)
		if(KeepWaveList[itor] > 0.5)
			CurrentPoint = itor
			SetPointScales()
			Get3by3BeamAverage()
	...
	// if nothing below current point, then start from end of list again
	for(itor = dimsize(KeepWaveList,0) - 1; itor >= 0; itor--)
		if(KeepWaveList[itor] > 0.5)
			CurrentPoint = itor
			SetPointScales()
...
// advance to the next point in subtraction list
Function SubtractionNextPoint(CtrlName) : ButtonControl
	String CtrlName
	...
	// check if we are at the end of the list, if so advance to the first point at the beginning that matches
	if(CurrentPoint > (dimsize(KeepWaveList,0) - 1))
		CurrentPoint = GetFirstAcceptedIndex()
		SetPointScales()
		Get3by3BeamAverage()
		ChangeCursor(Frame)
		ResetResidual()
	...
	// advance to the next point in the list that is selected by the user
	// start with one point beyond current point 
	// if loop reaches the end of the list, start back at the first point
		
	for(itor = CurrentPoint + 1; itor <= dimsize(KeepWaveList,0) - 1; itor++)
		if(KeepWaveList[itor] > 0.5)
			CurrentPoint = itor
			SetPointScales()
			Get3by3BeamAverage()
	...
	// none beyond the point selected, restart at the beginning of the list
	CurrentPoint = GetFirstAcceptedIndex()
	SetPointScales()
	Get3by3BeamAverage()
	ChangeCursor(Frame)
...
// set scale value on points in graphs based on location bracketing them in a 10x10 box
// this originally was a zoom, but slows down too much for large images
// switching to redimension images
Function SetPointScales()
	svar ChannelTwoReference
	svar ChannelOneReference
	svar ChannelTwoCropReference
	svar ChannelOneCropReference
	nvar CurrentPointIndex = root:SubtractionInterface:CurrentPointIndex
	nvar PixDelta = root:SubtractionInterface:PixelDelta
	wave X_Points = root:WaveNameArray_X
	wave Y_Points = root:WaveNameArray_Y

	Duplicate /O/R=[X_Points[CurrentPointIndex] - PixDelta,X_Points[CurrentPointIndex] + PixDelta][Y_Points[CurrentPointIndex] - PixDelta,Y_Points[CurrentPointIndex] + PixDelta] $ChannelOneReference,$ChannelOneCropReference
	Duplicate /O/R=[X_Points[CurrentPointIndex] - PixDelta,X_Points[CurrentPointIndex] + PixDelta][Y_Points[CurrentPointIndex] - PixDelta,Y_Points[CurrentPointIndex] + PixDelta] $ChannelTwoReference,$ChannelTwoCropReference
End
...
End
	...
	// reset the subtraction fit
Function ResetSubtract(CtrlName) : ButtonControl
	String CtrlName
...
Function AutoSubtract(CtrlName) : ButtonControl
	String CtrlName

	Wave Ch1_Average = root:SubtractionInterface:ChannelOne_Average
	Wave Ch2_Average = root:SubtractionInterface:ChannelTwo_Average
	Wave Ch_Res = root:SubtractionInterface:ChannelResidual
	nvar ChMult = root:SubtractionInterface:Channel_multiplier
	nvar ChOffset = root:SubtractionInterface:Channel_offset
	nvar MillisBetween = root:SubtractionInterface:PointIntervalLength
	nvar Ch_Res_exists = root:SubtractionInterface:ChannelResidual_exists
	nvar MaxAxis = root:SubtractionInterface:MaxLeftAxis
	nvar MinAxis = root:SubtractionInterface:MinLeftAxis
	nvar Frame = root:SubtractionInterface:Current_frame
	...
	// reset the channels if this function has been called before
	Get3by3BeamAverage()
	...
	//------------------------------------------------------------------------------------------------------------------------------------
	// OK lets try baseline subtraction before curve fitting to see if this simplifies things for us
	// igor has median boxcar smoothing which is similar in some respects to 
	// Friedrichs, M. S., A model-free algorithm for the removal of baseline artifacts. Journal of Biomolecular NMR 1995, 5 (2), 147–153.
	// with about 1/3 of the total points, median filter seems to approximate a baseline *very* well, so plan on using this
	//------------------------------------------------------------------------------------------------------------------------------------
	String savedDataFolder = GetDataFolder(1)	// Save
	SetDataFolder root:SubtractionInterface:
	
	// get the baseline 
	duplicate /O Ch1_Average Ch1_baseline
	smooth /M=0 (floor(numPnts(Ch1_baseline)/3)), Ch1_baseline
	//subtract it
	Ch1_Average = Ch1_Average - Ch1_baseline

	// get the baseline 
	duplicate /O Ch2_Average Ch2_baseline
	smooth /M=0 (floor(numPnts(Ch2_baseline)/3)), Ch2_baseline
	//subtract it
	Ch2_Average = Ch2_Average - Ch2_baseline
	...
	//---------------------------------------------------------------------------------------------
	// lets try to optimize one at a time, the minima calculation doesn't exactly work as expected
	//---------------------------------------------------------------------------------------------

	Make /O/N=(1,2) ParameterRangeSA
	ParameterRangeSA[0][0] = 0
	ParameterRangeSA[0][1] = 10
	Make /O/N=1 ParameterSolution
	ParameterSolution[0] = ChMult
	Optimize /M={3,0} /TSA={0,0.05} /XSA=ParameterRangeSA /X=ParameterSolution PeakMinimaCalculation,DummyWave

	ChMult = ParameterSolution[0] 

	Wave Ch2Shift = TimeShiftedWave(Ch2_Average, MillisBetween, ChOffset)

	Ch_Res = Ch1_Average - (Ch2Shift * ChMult)
	Ch_Res_exists = 1
	...
	// zoom out by 10 percent on the vertical data to avoid hitting the top and bottom for ease of viewing
	// get the maximum value for the two waves displayed
	variable maxVal = max((wavemax(Ch1_average)), (wavemax(Ch2_average)), (wavemax(Ch_Res)))
	variable minVal = min((wavemin(Ch1_average)), (wavemin(Ch2_average)), (wavemin(Ch_Res)))
	variable range = maxVal - minVal
	variable delta = range * 0.1 / 2.0
	MaxAxis = maxVal + delta
	MinAxis = minVal - delta
	SetAxis /W=ChannelSubtraction#IntensityTraces/Z left,MinAxis,MaxAxis 	
	SetAxis /W=ChannelSubtraction#IntensityTraces/Z right,MinAxis,MaxAxis 
	// color the residual blue and as a solid line
	ModifyGraph /W=ChannelSubtraction#IntensityTraces rgb(ChannelResidual)=(0,0,65535),lstyle(ChannelResidual)=0, lSize(ChannelResidual)=2
	//update cursor
	ChangeCursor(Frame)
	ChangeCursor(Frame)
	DoUpdate
...
Function PeakMinimaCalculation(DummyWave, Multiplier)
	Wave DummyWave
	Variable Multiplier
	...
	Make /O/N=(numpnts(Ch1_Average)) ToIntegrate 

	Wave Ch2Shift = TimeShiftedWave(Ch2_Average, MillisBetween, ChOffset)	

	ToIntegrate = (Ch1_Average - (Ch2Shift * Multiplier))

	Integrate ToIntegrate /D=ReturnValue 

	return abs(ReturnValue[numpnts(ReturnValue) - 1])

End



// to extrapolate the value of the data between points, using a linear function
// create a new wave that is time shifted by that value
// the time shift is assumed to be a positive value
// for the last value in the list, just assume the shift is equal to the last value (no point to draw a line with)	
Function/Wave TimeShiftedWave(ChannelToShift, MillisecondsPerInterval, MillisecondShift)
	Wave ChannelToShift
	Variable MillisecondsPerInterval, MillisecondShift


	//assert(MillisecondShift > 0)
	Assert(MillisecondsPerInterval > 0)

	String savedDataFolder = GetDataFolder(1)	// Save
	SetDataFolder root:SubtractionInterface:
	
	string ShiftedChannelRef = "ShiftedChannel"

	Make /O/N=(numpnts(ChannelToShift)) $ShiftedChannelRef
	
	Wave ShiftedChannel = $ShiftedChannelRef

	variable slope,intercept

	variable itor
	for(itor = 0; itor < numpnts(ShiftedChannel); itor++)

		// last value break 
		if(itor > (numpnts(ShiftedChannel) - 2))
			ShiftedChannel[itor] = ChannelToShift[itor]
			break
		EndIf

		slope = (ChannelToShift[itor] - ChannelToShift[itor+1]) / MillisecondsPerInterval
			
		intercept = ChannelToShift[itor] - (slope * itor * MillisecondsPerInterval)

		ShiftedChannel[itor] = slope * (itor * MillisecondsPerInterval + MillisecondShift) + intercept
...
Function ResetResidual()
	...
	Ch_Res = 0
	SetAxis /W=ChannelSubtraction#IntensityTraces/Z right,-1,1
	Ch_Res_exists = 0
...
Function UpdateResidual()
	...
	variable maxVal = wavemax(Ch_Res)
	variable minVal = wavemin(Ch_Res)
	variable range = maxVal - minVal
	variable delta = range * 0.1 / 2.0	
	SetAxis /W=ChannelSubtraction#IntensityTraces/Z right,(minVal - delta),(maxVal + delta)	
...
//---------------------------------------------------------------------------------------------------------------------------------------------
//---------------------------------------------------------------------------------------------------------------------------------------------
// Functions related to tracks
//---------------------------------------------------------------------------------------------------------------------------------------------
//---------------------------------------------------------------------------------------------------------------------------------------------

Function /WAVE GetTrackLengthHistogram(W_TracksWave)
	wave /WAVE W_TracksWave
	
	variable nTracks = DimSize(W_TracksWave, 0)
	
	//Make /O/N=nTracks TrackLengthWave
	Make /O/FREE/N=0 HistogramWave
	HistogramWave = 0

	variable itor,nPosInTrack

	// get each track then determine size
	for (itor = 0; itor < nTracks; itor++)
		wave theseTracks = W_TracksWave[itor]
		nPosInTrack = DimSize(theseTracks, 0)

		// if the track length is larger than the number of
		// locations in the histogram wave, then enlarge it
		// add the point, then add one to that location
		if(nPosInTrack >= numpnts(HistogramWave))
			redimension /N=(nPosInTrack+1) HistogramWave
			HistogramWave[nPosInTrack]+=1
			continue 
		endif
	...
	return HistogramWave

End
//---------------------------------------------------------------------------------------------------------------------------------------------
//---------------------------------------------------------------------------------------------------------------------------------------------
// to get histograms of velocities, the following function is employed
//---------------------------------------------------------------------------------------------------------------------------------------------
//---------------------------------------------------------------------------------------------------------------------------------------------

Function VelocityHistogramDialog(W_TracksWave)
	wave /WAVE W_TracksWave
	

	// set up a place to store the histogram data and/or clean it up if it already exists (maybe not)
	if(DataFolderExists("root:TrackHistogramData"))
		SetDataFolder root:TrackHistogramData
		KillWaves/A/Z			
	else
		NewDataFolder root:TrackHistogramData
		SetDataFolder root:TrackHistogramData		
	endif

	// create some variables for the user interface
	variable /G TrackLengthSelection = 2
	variable /G TimeStepSelection = 1
	variable /G BinSizeSelection = 10
	variable /G BinDeltaSelection = 1
	// duplicate the track wave to use it globally
	duplicate /O W_TracksWave GlobalTrack
	// make a histogram wave to use globally with default values to be modified later
	Make /O/N=10 AverageVelocityHistogramYAxis
	Make /O/N=10 AverageVelocityHistogramXAxis
	variable Itor
	for(Itor = 0; Itor < 10; Itor++)
		AverageVelocityHistogramXAxis[Itor] = Itor		
	endfor
	// information about the bin size in time units
	variable /G BinDeltaSeconds = 0

	// make a 2D wave containing X,Y, and T columns to make a 3D histogram
	// also make a second wave for a color profile
	Make /O/N=(0,3) TrajScatterPlot
	Make /O/N=(0,4) TrajScatterPlotColor
	variable /G ScattPlotMade = 0

	// make a trace for the maxwell-boltzmann equation fit 
	Make /O/N=0 MaxwellCurveFitX
	Make /O/N=0 MaxwellCurveFitY
	Variable /G MaxwellFitParameter = 0
	Variable /G MaxwellFitNormParameter = 0

	// make a trace for the gamma distribution equation fit
	variable /G GammaFitKParameter = 0
	variable /G GammaThetaParameter = 0
	Make /O/N=0 GammaCurveFitX
	Make /O/N=0 GammaCurveFitY

	// make a trace for the lognormal distribution equation fit
	variable /G LogNormalFitMuParameter = 0
	variable /G LogNormalFitSigmaParameter = 0
	Make /O/N=0 LogNormalCurveFitX
	Make /O/N=0 LogNormalCurveFitY

	// switch to record if PP plots have been displayed already
	variable /G PPPlotMade = 0
	

	// used to get the maximum size of the tracks for the "track length" set variable 
	Wave LengthHistogram = GetTrackLengthHistogram(W_TracksWave)

	NewPanel /W=(272,124,781,394) /N=AverageVelocityHistogram

	SetVariable TrackLengthSet,pos={357.00,15.00},size={134.00,19.00},title="Track Length",limits={2,numpnts(LengthHistogram) - 1,1},proc=TrackLengthSetVar,value=root:TrackHistogramData:TrackLengthSelection

	SetVariable TimeStepSet,pos={357.00,45.00},size={134.00,19.00},title="Time Step (s)",limits={1,inf,.001},proc=TimeStepSetVar,value=root:TrackHistogramData:TimeStepSelection

	SetVariable NumberBinSet,pos={341.00,75.00},size={150.00,19.00},title="Number of Bins",limits={1,inf,1},proc=NumberBinSetVar,value=root:TrackHistogramData:BinSizeSelection

	Button CalculateHistogram,pos={372.00,105.00},size={100.00,20.00},title="Calculate",proc=CalculateVelocityHistogram
	Button ExportHistogram,pos={372.00,135.00},size={100.00,20.00},title="Export",proc=ExportVelocityHistogram,disable=2

	Display/W=(12,11,327,258)/HOST=# AverageVelocityHistogramYAxis vs AverageVelocityHistogramXAxis
	RenameWindow #,Histogram

	// make the plot look like a histogram even though it's not

	ModifyGraph /W=AverageVelocityHistogram#Histogram mode(AverageVelocityHistogramYAxis)=5,hbFill(AverageVelocityHistogramYAxis)=2,useBarStrokeRGB(AverageVelocityHistogramYAxis)=1


	SetAxis /W=AverageVelocityHistogram#Histogram /A=1 bottom
	SetAxis /W=AverageVelocityHistogram#Histogram /A=1 left

	// set the line thickness
	ModifyGraph /W=AverageVelocityHistogram#Histogram lSize=2
	// set mirror axis with a thickness of 3, tick inside, major tick length 4, major tick thick 3, no minor tick, no axis standoff, no labels on top and right axis 
	ModifyGraph /W=AverageVelocityHistogram#Histogram mirror=1,axThick=3,tick=2,btLen=4,btThick=3,minor=0,standoff=0//,tkLblRot(bottom)=90
	Label bottom "Velocity Bin"
	Label left "Number Of Events"
	ModifyGraph /W=AverageVelocityHistogram#Histogram lblMargin(left)=10,lblMargin(bottom)=5

	// display the histogram curve fit (right axis)
	AppendToGraph /W=AverageVelocityHistogram#Histogram /R MaxwellCurveFitY vs MaxwellCurveFitX
	AppendToGraph /W=AverageVelocityHistogram#Histogram /R GammaCurveFitY vs GammaCurveFitX
	AppendToGraph /W=AverageVelocityHistogram#Histogram /R LogNormalCurveFitY vs LogNormalCurveFitX
	ModifyGraph /W=AverageVelocityHistogram#Histogram axThick(right)=3,tick(right)=2,btLen(right)=4,btThick(right)=3,minor(right)=0,standoff(right)=0
	ModifyGraph /W=AverageVelocityHistogram#Histogram lsize(MaxwellCurveFitY)=2,rgb(MaxwellCurveFitY)=(0,0,65535)
	ModifyGraph /W=AverageVelocityHistogram#Histogram lsize(GammaCurveFitY)=2,rgb(GammaCurveFitY)=(3,52428,1)
	ModifyGraph /W=AverageVelocityHistogram#Histogram lsize(LogNormalCurveFitY)=2,rgb(LogNormalCurveFitY)=(65535,52428,1)
...
Function TrackLengthSetVar(sva) : SetVariableControl
	STRUCT WMSetVariableAction &sva

	nvar TrackLengthSelect = root:TrackHistogramData:TrackLengthSelection

	switch( sva.eventCode )
		case 1: // mouse up
		case 2: // Enter key
		case 3: // Live update
			TrackLengthSelect = sva.dval
			break
	endswitch
...
Function TimeStepSetVar(sva) : SetVariableControl
	STRUCT WMSetVariableAction &sva

	nvar TimeStepSelect = root:TrackHistogramData:TimeStepSelection

	switch( sva.eventCode )
		case 1: // mouse up
		case 2: // Enter key
		case 3: // Live update
			TimeStepSelect = sva.dval
			break
	endswitch
...
Function NumberBinSetVar(sva) : SetVariableControl
	STRUCT WMSetVariableAction &sva

	nvar BinSizeSelect = root:TrackHistogramData:BinSizeSelection

	switch( sva.eventCode )
		case 1: // mouse up
		case 2: // Enter key
		case 3: // Live update
			BinSizeSelect = sva.dval
			break
	endswitch
...
Function CalculateVelocityHistogram(CtrlName) : ButtonControl
	String CtrlName

	Wave Track = root:TrackHistogramData:GlobalTrack
	nvar Length = root:TrackHistogramData:TrackLengthSelection
	nvar TimeStep = root:TrackHistogramData:TimeStepSelection
	nvar BinNumber = root:TrackHistogramData:BinSizeSelection
	nvar PPPlotMade = root:TrackHistogramData:PPPlotMade
	// Calculate the new histogram
	GetAverageVelocityHistogram(Track, Length,TimeStep, BinNumber)

	// fit to a maxwell-boltzmann equation
	// the K value will be known after this
	FitToMaxwellDistribution(Track,Length,TimeStep)

	// fit to a gamma distribution function for comparison
	FitToGammaDistribution(Track,Length,TimeStep)

	// fit to a log normal distribution function for comparison
	FitToLogNormalDistribution(Track,Length,TimeStep)

	// prepare the P-P test Comparisons
	// https://en.wikipedia.org/wiki/P%E2%80%93P_plot

	// get the velocity and caulculate the empirical CDF
	Wave AveragedVelocities = GetAverageVelocity(Track, Length,TimeStep)
	Duplicate /O AveragedVelocities LocalAveragedVelocities
	variable MaximumValue = Wavemax(LocalAveragedVelocities)

	Wave ECDW = ReturnEmpiricalCDW(LocalAveragedVelocities, BinNumber)
	Duplicate /O ECDW LocalECDW

	// get the theory CDFs

	// Maxwell
	nvar MaxwellK = root:TrackHistogramData:MaxwellFitParameter
	Wave MaxCDW = ReturnMaxwellCDW(MaxwellK, MaximumValue, BinNumber)
	Duplicate /O MaxCDW LocalMaxCDW

	// Gamma
	nvar GammaK = root:TrackHistogramData:GammaFitKParameter
	nvar GammaTheta = root:TrackHistogramData:GammaThetaParameter
	Wave GammaCDW = ReturnGammaCDW(GammaK, GammaTheta, MaximumValue, BinNumber)
	Duplicate /O GammaCDW LocalGammaCDW

	// LogNormal
	nvar LogNormMu = root:TrackHistogramData:LogNormalFitMuParameter
	nvar LogNormSigma = root:TrackHistogramData:LogNormalFitSigmaParameter
	Wave LogNormCDW = ReturnLogNormalCDW(LogNormMu,LogNormSigma, MaximumValue, BinNumber)
	Duplicate /O LogNormCDW LocalLogNormCDW

	// display plots (if they haven't already been displayed)
	if(!PPPlotMade)
		Display/W=(12,11,327,258) /N=MaxwellPP LocalECDW vs LocalMaxCDW
		Display/W=(12,11,327,258) /N=GammaPP LocalECDW vs LocalGammaCDW
		Display/W=(12,11,327,258) /N=LogNormPP LocalECDW vs LocalLogNormCDW
		PPPlotMade = 1
	Endif

	// get R2 performance for each
	variable RMax,RGamma,RLogNormal
	RMax = ReturnLinearR2(LocalMaxCDW,LocalECDW)
	RGamma = ReturnLinearR2(LocalGammaCDW,LocalECDW)
	RLogNormal = ReturnLinearR2(LocalLogNormCDW,LocalECDW)
	Print "R^2 values, Maxwell: " + num2str(RMax) + " Gamma: " + num2str(RGamma) +" LogNormal: " + num2str(RLogNormal)
	...
	SetAxis /W=AverageVelocityHistogram#Histogram /A=1 bottom
	SetAxis /W=AverageVelocityHistogram#Histogram /A=1 left
	SetAxis /W=AverageVelocityHistogram#Histogram /A=1 right

	Button ExportHistogram,disable=0
End




Function ExportVelocityHistogram(CtrlName) : ButtonControl
	String CtrlName

	Button ExportHistogram,disable=2
End

Function GetAverageVelocityHistogram(W_TracksWave,TrackLength,TimeLength, NumberToBin)
	wave /WAVE W_TracksWave
	variable TrackLength, TimeLength,NumberToBin 

	Wave AverageVelocityHistogramX = root:TrackHistogramData:AverageVelocityHistogramXAxis
	Wave AverageVelocityHistogramY = root:TrackHistogramData:AverageVelocityHistogramYAxis

	nvar DeltaToStore = root:TrackHistogramData:BinDeltaSeconds

	// account for the maximum being equal to the number to bin in the indexed location
	Redimension /N=(NumberToBin+1) AverageVelocityHistogramX
	Redimension /N=(NumberToBin+1) AverageVelocityHistogramY

	Wave AveragedVelocities = GetAverageVelocity(W_TracksWave,TrackLength,TimeLength)


	// blank the Y component of the histogram and start filling it up based on the average data bins
	AverageVelocityHistogramY = 0
	variable MaximumValue = wavemax(AveragedVelocities)
	variable Delta = MaximumValue / NumberToBin
	DeltaToStore = Delta
	variable Itor, Index
	for(Itor = 0; Itor < numpnts(AveragedVelocities); Itor++)
		Index = floor(AveragedVelocities[Itor] / Delta)
		AverageVelocityHistogramY[Index]+=1
	endfor
	// fill in the x component of the histogram (the Bin)
	for(Itor = 0; Itor < numpnts(AverageVelocityHistogramX); Itor++)
		AverageVelocityHistogramX[Itor] = Itor * Delta
	endfor

End
// get the  average indiviual velocity of all tracks of length "Tracklength"
// the "Timelength" is the period of time between frames
// "W_TracksWave" is the wave where all the tracks are stored 
Function /Wave GetAverageVelocity(W_TracksWave,TrackLength,TimeLength)
	wave /WAVE W_TracksWave
	variable TrackLength, TimeLength

	Wave TrajScatterPlot = root:TrackHistogramData:TrajScatterPlot
	Wave TrajScatterPlotColor = root:TrackHistogramData:TrajScatterPlotColor

	variable nTracks = DimSize(W_TracksWave, 0)
	variable nSpecificTracks = 0
	Make /O/FREE/N=(nSpecificTracks) VelocityWave
	

	variable Itor, AveragingItor, Velocity, AverageVelocity,xCol, yCol, zCol, X_1, X_2, Y_1, Y_2, TrackDistance

	// move through all tracks, looking for tracks that match the track length requested
	for (Itor = 0; Itor < nTracks; Itor++)
		wave theseTracks = W_TracksWave[Itor]


		if(DimSize(theseTracks, 0) == TrackLength)

			nSpecificTracks+=1
			redimension /N=(nSpecificTracks) VelocityWave

			// required by localizer, array index to coordinates is object specific 
			GetColumnsForEmitterPositions(theseTracks, xCol, yCol, zCol)			


			// creating a scatter plot with the x,y,and time (of the first point in the track)	
			redimension /N=(nSpecificTracks,3) TrajScatterPlot
			TrajScatterPlot[nSpecificTracks - 1][0] = theseTracks[0][xCol]
			TrajScatterPlot[nSpecificTracks - 1][1] = theseTracks[0][yCol]
			TrajScatterPlot[nSpecificTracks - 1][2] = theseTracks[0][0] * TimeLength

			// average the velocity for each individual track
			AverageVelocity = 0
			for(AveragingItor = 0; AveragingItor < (TrackLength-1); AveragingItor++)
				X_1 = theseTracks[AveragingItor][xCol]
				X_2 = theseTracks[AveragingItor+1][xCol]
				Y_1 = theseTracks[AveragingItor][yCol]
				Y_2 = theseTracks[AveragingItor+1][yCol]
				TrackDistance = sqrt((X_2 - X_1)^2 + (Y_2 - Y_1)^2)

				Velocity = TrackDistance / TimeLength

				// add the new velocity to the averaged velocity sum
				AverageVelocity = (AverageVelocity * AveragingItor + Velocity) / (AveragingItor +1)
			endfor

			VelocityWave[nSpecificTracks - 1] = AverageVelocity
	...
	// green to yellow to red makes sense
	// alpha is 1 the whole time?

	// first get the maximum and minimum velocities
	variable MaxSpeed = 0, MinSpeed = 0
	MinSpeed = wavemin(VelocityWave)
	MaxSpeed = wavemax(VelocityWave)

	// match the color wave size to the scatter plot size
	redimension /N=(DimSize(TrajScatterPlot,0),4) TrajScatterPlotColor
	...
	if(MaxSpeed > MinSpeed)
		...
		variable ColorScaler, Red,Green,Blue = 0,Alpha = 1
		for(Itor = 0; Itor < DimSize(TrajScatterPlotColor,0); Itor++)
			ColorScaler =  (VelocityWave[Itor] - MinSpeed) / (MaxSpeed - MinSpeed)
			Red = 2.0 * ColorScaler
			Green = 2.0 * (1 - ColorScaler)
			// rescale when color is > 1.0
			if(Red > 1.0)
				Red = 1.0
			endif
			if(Green > 1.0)
			...
			TrajScatterPlotColor[Itor][0] = Red
			TrajScatterPlotColor[Itor][1] = Green
			TrajScatterPlotColor[Itor][2] = Blue
			TrajScatterPlotColor[Itor][3] = Alpha

		endfor
	else
		for(Itor = 0; Itor < DimSize(TrajScatterPlotColor,0); Itor++)
			TrajScatterPlotColor[Itor][0] = 1.0
			TrajScatterPlotColor[Itor][1] = 0.0
			TrajScatterPlotColor[Itor][2] = 0.0
			TrajScatterPlotColor[Itor][3] = 1.0
		endfor
	endif	
...
//---------------------------------------------------------------------------------------------------
// Function returns an R^2 value assuming the data 
// fits to a line with an intercept of 0 and slope of 1
//---------------------------------------------------------------------------------------------------
Function ReturnLinearR2(XData,YData)
	wave XData,YData


	variable Itor,Residual,Total,MeanData

	MeanData = 0
	for(Itor = 0; Itor < numpnts(YData); Itor++)
	if(numtype(YData[Itor]) == 2)
		continue
	endif
		MeanData = MeanData + YData[Itor]
	Endfor
	MeanData = MeanData / numpnts(YData)

	Residual = 0
	//F(x) is equal to X for a slope of one so just subtract X from Y(experimental)
	for(Itor = 0; Itor < numpnts(XData); Itor++)
	if((numtype(YData[Itor]) == 2)||(numtype(XData[Itor]) == 2))
		continue
	endif
		Residual = (YData[Itor] - XData[Itor])^2 + Residual
	Endfor
	...
	for(Itor = 0; Itor < numpnts(XData); Itor++)
		...
		Total = (YData[Itor] - MeanData)^2 + Total
	Endfor
...
//---------------------------------------------------------------------------------------------------
// Function returns a cumulative distribution wave based on the number of bins and the input data  
//---------------------------------------------------------------------------------------------------
Function /WAVE ReturnEmpiricalCDW(RawData, NumPoints)
	wave RawData
	variable NumPoints

	make /O/N=(numpnts(RawData)) SortedRawData

	Sort RawData RawData

	// need +1 to include the end data point in the list
	Make /O/FREE/N=(NumPoints + 1) ECDW

	variable Delta = wavemax(RawData) / NumPoints

	variable Itor

	// get all the events
	
	for(Itor = 0; Itor <= NumPoints; Itor++)
		ECDW[itor] = CumulativeCalc(RawData,Itor * Delta) / numpnts(RawData)
	Endfor

	return ECDW
End	

Function CumulativeCalc(SortedData,CutoffValue)
	Wave SortedData
	variable CutoffValue

	variable CutoffSum = 0

	variable Itor
	for(Itor = 0; Itor < numpnts(SortedData); Itor++)
		if(SortedData[Itor] <= CutoffValue)
			CutoffSum += 1
		else
			break
		endif
...
//---------------------------------------------------------------------------------------------------
// this mini section is to fit data to a maxwell-boltzmann function, appropriate for moving particles
//---------------------------------------------------------------------------------------------------


	root:TrackHistogramData:MaxwellFitData
	


Function FitToMaxwellDistribution(W_TracksWave,TrackLength,TimeLength)
	Wave W_TracksWave
	variable TrackLength,TimeLength


	nvar MaxParam = root:TrackHistogramData:MaxwellFitParameter
	nvar NormConstant = root:TrackHistogramData:MaxwellFitNormParameter
	Wave FitX = root:TrackHistogramData:MaxwellCurveFitX
	Wave FitY = root:TrackHistogramData:MaxwellCurveFitY


	// calculate the fit of a maxwell-boltzmann equation to the data
	Wave AveragedVelocities = GetAverageVelocity(W_TracksWave,TrackLength,TimeLength)
	optimize /A=1/m={2,1}/L=0/H=(10*Wavemax(AveragedVelocities)) MaxwellFitFunction,AveragedVelocities 
	MaxParam = V_maxloc

	Redimension /N=500 FitX 
	Redimension /N=500 FitY

	variable DeltaPlot = wavemax(AveragedVelocities) / 500

	NormConstant = NormMaxwellDist(AveragedVelocities,MaxParam)
	variable Itor
	For(Itor = 0; Itor < 500; Itor++)
		FitX[Itor] = DeltaPlot * Itor
		FitY[Itor] = StatsMaxwellPDF(DeltaPlot * Itor, MaxParam) * NormConstant
	endfor

	
End

Function MaxwellFitFunction(XVals,K)
	Wave XVals
	Variable K

	variable NormConstant = NormMaxwellDist(XVals,K)
	variable Itor
	Variable Multiple = 0

	for(Itor = 0; Itor < numpnts(XVals); Itor++)
		Multiple = Multiple + ln(StatsMaxwellPDF(XVals[Itor], K) * NormConstant)
	Endfor

	return Multiple
End

Function NormMaxwellDist(RangeMax,K)
	wave RangeMax
	variable K

	// make the range of the integration 10 times the size of the last X value
	variable Max = WaveMax(RangeMax) * 100
	// create waves to integrate over that region for normalization
	Make /O/N=1001 XIntWave // required by integrator
	Make /O/N=1000 YIntWave	
	Make /O/N=1000 IntegrationResult
	// determine the size of each x step based on a 500 point wave starting at zero
	variable Delta = Max / 1000
	// fill in the values for the X and Y wave
	variable Itor
	for(Itor = 0; Itor < 1000; Itor++)
		YIntWave[Itor] = StatsMaxwellPDF(Itor * Delta, K)
		XIntWave[Itor] = Itor * Delta
	endfor
	// fill in the last point for the integrator
	XIntWave[1000] = Delta * 1000 

	// do the integration
	Integrate YIntWave /X=XIntWave /D=IntegrationResult
	// return the normalization constant (1 / total integration)
	return (1 / IntegrationResult[numpnts(IntegrationResult) - 1]) 
End


// return a CDF function wave for the Maxwell equation 

Function /WAVE ReturnMaxwellCDW(KValue, MaximumValue, NumPoints)
	variable KValue, MaximumValue, NumPoints

	// need +1 to include the end data point in the list
	Make /O/FREE/N=(NumPoints + 1) MaxCDW

	variable Delta = MaximumValue / NumPoints

	variable Itor

	// get all the events
	for(Itor = 0; Itor <= NumPoints; Itor++)
		MaxCDW[itor] = StatsMaxwellCDF(Itor * Delta, KValue)
	Endfor
...
//---------------------------------------------------------------------------------------------------
// this mini section is to fit data to a gamma distribution function
// https://en.wikipedia.org/wiki/Gamma_distribution
//---------------------------------------------------------------------------------------------------



Function FitToGammaDistribution(W_TracksWave,TrackLength,TimeLength)
	Wave W_TracksWave
	...
	nvar GammaK = root:TrackHistogramData:GammaFitKParameter
	nvar GammaTheta = root:TrackHistogramData:GammaThetaParameter
	Wave GammaFitX = root:TrackHistogramData:GammaCurveFitX
	Wave GammaFitY = root:TrackHistogramData:GammaCurveFitY

	Make /O/N=2 GammaParameterLimits

	// calculate the fit of a gamma distribution to the data
	Wave AveragedVelocities = GetAverageVelocity(W_TracksWave,TrackLength,TimeLength)
	GammaParameterLimits[0] = 0.00001
	GammaParameterLimits[1] = 100*Wavemax(AveragedVelocities)	
	optimize /A=1/m={2,1}/X=GammaParameterLimits GammaFitFunction,AveragedVelocities 
	GammaK = GammaParameterLimits[0]
	GammaTheta = GammaParameterLimits[1]

	Redimension /N=500 GammaFitX 
	Redimension /N=500 GammaFitY

	// to display the result of the gamma fit
	variable DeltaPlot = wavemax(AveragedVelocities) / 500
	variable Itor
	For(Itor = 0; Itor < 500; Itor++)
		GammaFitX[Itor] = DeltaPlot * Itor
		GammaFitY[Itor] = GammaPDF(DeltaPlot * Itor,GammaK,GammaTheta)
	endfor
...
Function GammaFitFunction(XVals,K,Theta)
	Wave XVals
	Variable K,Theta
	...
	Variable Multiple = 0
	for(Itor = 0; Itor < numpnts(XVals); Itor++)
		Multiple = Multiple + ln(GammaPDF(XVals[Itor],K,Theta))
	Endfor
	return Multiple
End
...
Function GammaPDF(X,K,Theta)
	...
	variable GammaValue = Gamma(K)
	variable EqPart1 = 1 / (GammaValue * (Theta ^ K)) 
	variable EqPart2 = X^(K - 1)
	variable EqPart3 = e^(-X / Theta)

	return EqPart1 * EqPart2 * EqPart3
End


// return a CDF function wave for the Gamma equation 

Function /WAVE ReturnGammaCDW(KValue, ThetaValue, MaximumValue, NumPoints)
	variable KValue, ThetaValue, MaximumValue, NumPoints

	// need +1 to include the end data point in the list
	Make /O/FREE/N=(NumPoints + 1) GammaCDW
	...
	// get all the events
	for(Itor = 0; Itor <= NumPoints; Itor++)
		GammaCDW[itor] = GammaCumulativeCalc(Itor * Delta, KValue, ThetaValue)
	Endfor
...
Function GammaCumulativeCalc(X_Value, KValue, ThetaValue)
	variable X_Value, KValue, ThetaValue

	variable GammaValue = Gamma(KValue)
	variable LowerIncGamma = gammaInc(KValue,X_Value / ThetaValue,0)

	return LowerIncGamma / GammaValue
...
//---------------------------------------------------------------------------------------------------
// this mini section is to fit data to a log-normal distribution function
// https://en.wikipedia.org/wiki/Log-normal_distribution
//---------------------------------------------------------------------------------------------------
Function FitToLogNormalDistribution(W_TracksWave,TrackLength,TimeLength)
	Wave W_TracksWave
	variable TrackLength,TimeLength
	...
	nvar LogNormMu = root:TrackHistogramData:LogNormalFitMuParameter
	nvar LogNormSigma = root:TrackHistogramData:LogNormalFitSigmaParameter
	Wave LogNormFitX = root:TrackHistogramData:LogNormalCurveFitX
	Wave LogNormFitY = root:TrackHistogramData:LogNormalCurveFitY

	Make /O/N=2 LogNormalParameterLimits

	// calculate the fit of a gamma distribution to the data
	Wave AveragedVelocities = GetAverageVelocity(W_TracksWave,TrackLength,TimeLength)
	LogNormalParameterLimits[0] = 0.00001
	LogNormalParameterLimits[1] = 100*Wavemax(AveragedVelocities)	
	optimize /A=1/m={2,1}/X=LogNormalParameterLimits LogNormalFitFunction,AveragedVelocities 
	LogNormMu = LogNormalParameterLimits[0]
	LogNormSigma = LogNormalParameterLimits[1]

	Redimension /N=500 LogNormFitX 
	Redimension /N=500 LogNormFitY

	// to display the result of the gamma fit
	variable DeltaPlot = wavemax(AveragedVelocities) / 500
	variable Itor
	For(Itor = 0; Itor < 500; Itor++)
		LogNormFitX[Itor] = DeltaPlot * Itor
		LogNormFitY[Itor] = LogNormalPDF(DeltaPlot * Itor,LogNormMu,LogNormSigma)
	endfor


End

Function LogNormalFitFunction(XVals,Mu,Sigma)
	Wave XVals
	Variable Mu,Sigma
	...
	Variable Multiple = 0
	for(Itor = 0; Itor < numpnts(XVals); Itor++)
		Multiple = Multiple + ln(LogNormalPDF(XVals[Itor],Mu,Sigma))
	Endfor
	return Multiple
End
...
Function LogNormalPDF(X,Mu,Sigma)
	...
	variable EqPart1 = 1 / (X * Sigma * sqrt(2*Pi)) 
	variable EqPart2 = e^((-1.0 * (Ln(X) - Mu)^2) / (2 * Sigma^2))
...
Function /WAVE ReturnLogNormalCDW(MuValue,SigmaValue, MaximumValue, NumPoints)
	variable MuValue, SigmaValue, MaximumValue, NumPoints

	// need +1 to include the end data point in the list
	Make /O/FREE/N=(NumPoints + 1) LogNormalCDW
	...
	// get all the events
	for(Itor = 0; Itor <= NumPoints; Itor++)
		LogNormalCDW[itor] = LogNormalCumulativeCalc(Itor * Delta, MuValue, SigmaValue)
	Endfor
...
Function LogNormalCumulativeCalc(X_Value, MuValue, SigmaValue)
	variable X_Value, MuValue, SigmaValue

	variable numerator, denominator

	numerator = MuValue - ln(X_Value)
	denominator = SigmaValue * sqrt(2)
...
//---------------------------------------------------------------------------------------------------------------------------------------------
//---------------------------------------------------------------------------------------------------------------------------------------------
// to get histograms of Diffusion rates, the following function is employed
//---------------------------------------------------------------------------------------------------------------------------------------------
//---------------------------------------------------------------------------------------------------------------------------------------------



Function MSDHistogramDialog(W_TracksWave)
	wave /WAVE W_TracksWave
	...
	// create some variables for the user interface
	variable /G MaximumTrackLength =  numpnts(LengthHistogram) - 1
	variable /G TrackBeginSelection = 2
	variable /G TrackEndSelection = MaximumTrackLength
	variable /G TimeFrameSelection = 1

	// duplicate the track wave to use it globally
	duplicate /O W_TracksWave GlobalTrack
	// make a histogram wave to use globally with default values to be modified later
	Make /O/N=10 MSDHistogramYAxis
	Make /O/N=10 MSDHistogramXAxis
	variable Itor
	for(Itor = 0; Itor < 10; Itor++)
		MSDHistogramXAxis[Itor] = Itor		
	endfor
	// information about the bin size in time units
	variable /G BinDeltaSeconds = 0
	...
	NewPanel /W=(272,124,781,394) /N=MSDHistogram

	SetVariable TrackBeginSet,pos={357.00,15.00},size={134.00,19.00},title="Track Length Min",limits={0,MaximumTrackLength,1},proc=TrackBeginMSDSetVar,value=root:TrackHistogramData:TrackBeginSelection

	SetVariable TrackEndSet,pos={357.00,45.00},size={134.00,19.00},title="Track Length Max",limits={0,MaximumTrackLength,1},proc=TrackEndMSDSetVar,value=root:TrackHistogramData:TrackEndSelection

	SetVariable TimeFrameSet,pos={357.00,75.00},size={134.00,19.00},title="Time Step (Frames)",limits={1,inf,1},proc=TimeStepMSDSetVar,value=root:TrackHistogramData:TimeFrameSelection

	Button CalculateHistogram,pos={372.00,105.00},size={100.00,20.00},title="Calculate",proc=CalculateMSDHistogram
	Button ExportHistogram,pos={372.00,135.00},size={100.00,20.00},title="Export",proc=ExportMSDHistogram,disable=2

	Display/W=(12,11,327,258)/HOST=# MSDHistogramYAxis vs MSDHistogramXAxis
	RenameWindow #,Histogram

	// make the plot look like a histogram even though it's not

	ModifyGraph /W=MSDHistogram#Histogram mode(MSDHistogramYAxis)=5,hbFill(MSDHistogramYAxis)=2,useBarStrokeRGB(MSDHistogramYAxis)=1


	SetAxis /W=MSDHistogram#Histogram /A=1 bottom
	SetAxis /W=MSDHistogram#Histogram /A=1 left

	// set the line thickness
	ModifyGraph /W=MSDHistogram#Histogram lSize=2
	// set mirror axis with a thickness of 3, tick inside, major tick length 4, major tick thick 3, no minor tick, no axis standoff, no labels on top and right axis 
	ModifyGraph /W=MSDHistogram#Histogram mirror=1,axThick=3,tick=2,btLen=4,btThick=3,minor=0,standoff=0//,tkLblRot(bottom)=90
	Label bottom "MSD Bin"
	Label left "Number Of Events"

	// display the histogram curve fit (right axis)
	//AppendToGraph /W=MSDHistogram#Histogram /R MaxwellCurveFitY vs MaxwellCurveFitX
	//AppendToGraph /W=MSDHistogram#Histogram /R GammaCurveFitY vs GammaCurveFitX
	//ModifyGraph /W=MSDHistogram#Histogram axThick(right)=3,tick(right)=2,btLen(right)=4,btThick(right)=3,minor(right)=0,standoff(right)=0,lsize(MaxwellCurveFitY)=2,rgb(MaxwellCurveFitY)=(0,0,65535),lsize(GammaCurveFitY)=2,rgb(GammaCurveFitY)=(3,52428,1)
...
Function TrackBeginMSDSetVar(sva) : SetVariableControl
	STRUCT WMSetVariableAction &sva

	nvar TrackBeginSelect = root:TrackHistogramData:TrackBeginSelection
	nvar TrackEndSelect =root:TrackHistogramData:TrackEndSelection

	switch( sva.eventCode )
		case 1: // mouse up
		case 2: // Enter key
		case 3: // Live update
			if(sva.dval < TrackEndSelect)
				TrackBeginSelect = sva.dval
			else
				if(TrackEndSelect > 1)
					TrackBeginSelect = TrackEndSelect - 1
				else
					TrackBeginSelect = 0
				endif	
			endif
...
Function TrackEndMSDSetVar(sva) : SetVariableControl
	STRUCT WMSetVariableAction &sva

	nvar TrackBeginSelect = root:TrackHistogramData:TrackBeginSelection
	nvar TrackEndSelect =root:TrackHistogramData:TrackEndSelection
	nvar TrackMax =root:TrackHistogramData:MaximumTrackLength
	switch( sva.eventCode )
		case 1: // mouse up
		case 2: // Enter key
		case 3: // Live update
			if(sva.dval > TrackBeginSelect)
				TrackEndSelect = sva.dval
			else
				if(TrackBeginSelect < TrackMax)
					TrackEndSelect = TrackBeginSelect + 1
				else
					TrackEndSelect = TrackMax
				endif	
			endif
			break
	endswitch
...
Function TimeStepMSDSetVar(sva) : SetVariableControl
	STRUCT WMSetVariableAction &sva

	nvar TimeFrameSelect = root:TrackHistogramData:TimeFrameSelection
	nvar TrackBeginSelect = root:TrackHistogramData:TrackBeginSelection
	...
	switch( sva.eventCode )
		...
		case 3: // Live update
			...
			if(sva.dval <= TimeFrameSelect) 
				TimeFrameSelect = sva.dval
			else
				TimeFrameSelect = TrackBeginSelect
			endif
			break
	endswitch
...
Function NumberBinMSDSetVar(sva) : SetVariableControl
	STRUCT WMSetVariableAction &sva
...
Function CalculateMSDHistogram(CtrlName) : ButtonControl
	String CtrlName

	Wave Track = root:TrackHistogramData:GlobalTrack
	nvar TrackBegin = root:TrackHistogramData:TrackBeginSelection
	nvar TrackEnd = root:TrackHistogramData:TrackEndSelection
	nvar NumberFrames = root:TrackHistogramData:TimeFrameSelection




	// Calculate the new histogram
	GetMSDHistogram(Track,TrackBegin,TrackEnd,NumberFrames)
	...
	SetAxis /W=MSDHistogram#Histogram /A=1 bottom
	SetAxis /W=MSDHistogram#Histogram /A=1 left
	//SetAxis /W=AverageMSDHistogram#Histogram /A=1 right
...
Function ExportMSDHistogram(CtrlName) : ButtonControl
	String CtrlName
...
Function GetMSDHistogram(W_TracksWave,TrackBegin,TrackEnd,NumberFrames)
	wave /WAVE W_TracksWave
	variable TrackBegin, TrackEnd,NumberFrames 

	Wave MSDHistogramX = root:TrackHistogramData:MSDHistogramXAxis
	Wave MSDHistogramY = root:TrackHistogramData:MSDHistogramYAxis

	// account for the maximum being equal to the number to bin in the indexed location
	Redimension /N=(TrackEnd + 1) MSDHistogramX
	Redimension /N=(TrackEnd + 1) MSDHistogramY

	MSDHistogramX = 0
	MSDHistogramX = 0

	variable Itor
	for(Itor = TrackBegin; Itor <= TrackEnd; Itor++)
		Wave SD = GetSD(W_TracksWave,Itor,NumberFrames)
		if(numpnts(SD) > 0)
			WaveStats /Q SD
			MSDHistogramY[Itor] = V_avg
		else
			MSDHistogramY[Itor] = 0
		endif	
		MSDHistogramX[Itor] = Itor 
	endfor  	

End

// return the square displacement of all tracks of a certain length
Function /Wave GetSD(W_TracksWave,TrackLength,NumberFrames)
	wave /WAVE W_TracksWave
	variable TrackLength, NumberFrames


	variable nTracks = DimSize(W_TracksWave, 0)
	variable nSpecificTracks = 0
	Make /O/FREE/N=(nSpecificTracks) SDWave
	

	variable Itor, SD,xCol, yCol, zCol, X_Init, X_Final, Y_Init, Y_Final
	...
	for (Itor = 0; Itor < nTracks; Itor++)
		...
		if(DimSize(theseTracks, 0) == TrackLength)
			...
			// calculate the Square Displacement when you find a wave

			nSpecificTracks+=1
			redimension /N=(nSpecificTracks) SDWave

			// required by localizer, array index to coordinates is object specific 
			GetColumnsForEmitterPositions(theseTracks, xCol, yCol, zCol)			
			
			X_Init = theseTracks[0][xCol]
			Y_Init = theseTracks[0][yCol]
		
			X_Final = theseTracks[NumberFrames][xCol]
			Y_Final = theseTracks[NumberFrames][yCol]
			SD = (X_Final - X_Init)^2 + (Y_Final - Y_Init)^2

			SDWave[nSpecificTracks - 1] = SD
...
// return the angular direction (in radians) of all tracks of a certain length
Function /Wave GetAngle(W_TracksWave,TrackLength,NumberFrames)
	wave /WAVE W_TracksWave
	variable TrackLength, NumberFrames
	...
	variable nTracks = DimSize(W_TracksWave, 0)
	variable nSpecificTracks = 0
	Make /O/FREE/N=(nSpecificTracks) AngleWave
	

	variable Itor, Angle,xCol, yCol, zCol, X_Init, X_Final, Y_Init, Y_Final
	...
	for (Itor = 0; Itor < nTracks; Itor++)
		...
		if(DimSize(theseTracks, 0) == TrackLength)
			...
			nSpecificTracks+=1
			redimension /N=(nSpecificTracks) AngleWave

			// required by localizer, array index to coordinates is object specific 
			GetColumnsForEmitterPositions(theseTracks, xCol, yCol, zCol)								

			AngleWave[nSpecificTracks - 1] = 0
